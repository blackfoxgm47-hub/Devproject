...
function calculateCabinetAverage(cabinet) {
    ...
    const statusElement = document.getElementById(`status-cabinet-${cabinet}`);
    if (statusElement) {
        statusElement.innerHTML = `<span class="${getStatusClass(status)}">${status}</span>`;
    }
...
function deleteRow(cabinet, rowIndex) {
    ...
    showToast('success', `ลบคันที่ ${displayRow} ของตู้ที่ ${cabinet} เรียบร้อย!`);
}
